...
function App() {
  // 数据层
  const { groups, loading, create, remove } = useSessions()
  const { poolStatus, activeId, setPoolStatus } = usePoolStatus()
  const { chatMessages, streaming, error, setError, sendMessage, clearMessages, replaceMessages } = useChat()
  ...
  return (
    <div className="h-dvh bg-white text-zinc-800 flex flex-col overflow-hidden">
      ...
      {/* 移动端顶部导航 */}
      <header className="md:hidden flex items-center gap-3 px-4 py-3 border-b border-zinc-200 bg-white flex-shrink-0 z-30">
